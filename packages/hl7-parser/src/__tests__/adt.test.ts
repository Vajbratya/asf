...
IN1|1|PLAN123|INS_CO|Insurance Company Name|||POLICY456|GROUP789`;
  ...
  describe('parse', () => {
    ...
    it('should throw error for missing PID', () => {
      ...
      expect(() => ADTParser.parse(message)).toThrow('ADT message missing PID segment');
    });
  });

  describe('getEventDescription', () => {
    it('should return description for known events', () => {
      expect(ADTParser.getEventDescription('A01')).toBe('Admit/Visit Notification');
      ...
      expect(ADTParser.getEventDescription('A08')).toBe('Update Patient Information');
    });
